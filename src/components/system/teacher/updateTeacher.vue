<template>
<div>
  <el-dialog title="编辑教师" width="40%" :destroy-on-close="true" :visible.sync="updateUserVisible" :before-close="handleClose">
        ...
        if(newVal) {
          getUserById({id:newVal}).then(res => {
            if(res.code == 1000) {
              ...
              this.fileList.push({url:this.$store.state.configure.HOST + this.form.flair})
            } else {
              this.$notify.error({
                title: '错误',
                message: res.message
