...
<script>
  import {getApeForumItemPage,removeApeForumItem} from '../../../api/api' 
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      query() {
        getApeForumItemPage(this.search).then(res => {
          if(res.code == 1000) {
      ...
      deleteDate(ids) {
        removeApeForumItem({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    mounted() {
      this.search.forumId = this.$route.query.id
      this.query()
    }
 }
</script>
